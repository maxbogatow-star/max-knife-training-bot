...
def callback(u, uid):
    ...
    if payload == "memo":
        return send(uid, "🎒 Что взять на тренировку\n\n"
                    ...
                    "Инвентарь для спортивного метания предоставляется организаторами.",
        ...
        try:
            ...
            for t0 in rs:
                ...
                if timedelta(hours=23) <= delta <= timedelta(hours=25):
                    ...
                    if claimed:
                        t=training(t0["id"])
                        msg=("⏰ Напоминание о тренировке\n\n"
                             "Завтра состоится тренировка по спортивному метанию ножа.\n\n"
                             +training_text(t)+
                             "\n\n⏱ Рекомендуем прибыть за 10–15 минут до начала.\n"
                             "Если ваши планы изменились, пожалуйста, отмените запись заранее.")
                        c2=conn()
